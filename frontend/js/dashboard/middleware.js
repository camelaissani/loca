...
class DashboardMiddleware extends BaseViewMiddleware {
  ...
  updated() {
    ...
    if (!this.notificationListTemplate) {
      this.notificationListTemplate = Handlebars.compile(
        $('#notification-list-template').html()
      );
    }
  ...
  _loadOccupantsOverview(callback) {
    application.httpGet('/api/occupants/overview', (req, res) => {
      ...
      $('#view-dashboard #count-all-occupants-label').html(
        i18next.t('Tenant', { count: countAll })
      );
      $('#view-dashboard #count-active-occupants-label').html(
        i18next.t('Lease', { count: countActive })
      );
      $('#view-dashboard #count-inactive-occupants-label').html(
        i18next.t('Terminated lease', { count: countActive })
      );
      if (callback) {
        callback();
      }
  ...
  _loadPropertiesOverview(callback) {
    application.httpGet('/api/properties/overview', (req, res) => {
      ...
      $('#view-dashboard #count-all-properties-label').html(
        i18next.t('Property', { count: countAll })
      );
      $('#view-dashboard #count-active-properties-label').html(
        i18next.t('Leased', { count: countBusy })
      );
      $('#view-dashboard #count-inactive-properties-label').html(
        i18next.t('Available', { count: countFree })
      );
      if (callback) {
        callback();
      }
  ...
  _loadRentsOverview(callback) {
    application.httpGet('/api/rents/overview', (req, res) => {
      ...
      $('#view-dashboard #count-all-rents-label').html(
        i18next.t('Rent', { count: countAll })
      );
      $('#view-dashboard #count-paid-rents').html(
        countPaidAndPartiallyPaid +
          ' ' +
          i18next.t('Rent', { count: countPaidAndPartiallyPaid })
      );
      $('#view-dashboard #count-not-paid-rents').html(
        countNotPaid + ' ' + i18next.t('Rent', { count: countNotPaid })
      );
      //$('#view-dashboard #count-partially-paid-rents').html(countPartiallyPaid);
      $('#view-dashboard #count-js-total-topay-rents').html(
        Helper.formatMoney(totalToPay)
      );
      $('#view-dashboard #count-total-notpaid-rents').html(
        Helper.formatMoney(totalNotPaid * -1)
      );
      $('#view-dashboard #count-total-paid-rents').html(
        Helper.formatMoney(totalPaid)
      );
      if (callback) {
        callback();
      }
  ...
  _loadNotifications(callback) {
    application.httpGet('/api/notifications', (req, res) => {
      ...
      $('#view-dashboard #carousel-notifications').html(
        this.notificationListTemplate({

...
class AccountingMiddleware extends ViewController {
  ...
  onInitTemplate() {
    ...
    Handlebars.registerPartial(
      ...
      $('#accounting-payments-row-template').html()
    );

    this.templatePaymentsTable = Handlebars.compile(
      $('#accounting-payments-template').html()
    );
    this.templateEntriesExitsTable = Handlebars.compile(
      $('#accounting-entries-exits-template').html()
    );
  }
  ...
  onDataChanged(callback) {
    ...
    $yearPicker.datepicker(
      'setDate',
      moment('01/01/' + this.year, 'DD/MM/YYYY').toDate()
    );
  ...
  load(callback) {
    application.httpGet(`/api/accounting/${this.year}`, (req, res) => {
      const data = JSON.parse(res.responseText);
      data.payments.months = moment.months();
      $('#view-accounting #accounting-payments-table').html(
        this.templatePaymentsTable(data.payments)
      );
      $('#view-accounting #accounting-entries-exits-table').html(
        this.templateEntriesExitsTable(data.entriesExists)
      );
      $('#view-accounting #accounting-payments-fake-table').width(
        $('#view-accounting #accounting-payments-per-year-table').outerWidth()
      );

      // bind top horizontal scrollbar with table
      const $topHScroll = $(
        '#view-accounting #accounting-payments-table-top-hscroll'
      );
      const $viewScroll = $(
        '#view-accounting #accounting-payments-per-year-table'
      ).parent();

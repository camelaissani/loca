...
class RentMiddleware extends ViewController {
  ...
  loadList(callback) {
    application.httpGet(
      ...
      (req, res) => {
        ...
        $('.js-paid-filter-label').html(
          '(' + (countPaid + countPartiallyPaid) + ')'
        );
        ...
        if (this.filterValue) {
          $(
            '#view-rent .js-filterbar .js-user-action[data-value="' +
              this.filterValue +
              '"]'
          ).addClass('active');
        } else {
          $(
            '#view-rent .js-filterbar .js-default-filter.js-user-action'
          ).addClass('active');
        }
  ...
  onInitTemplate() {
    ...
    Handlebars.registerPartial(
      ...
      $('#view-rent-payment-badges-template').html()
    );
    Handlebars.registerPartial(
      'view-rent-email-status-badges-template',
      $('#view-rent-email-status-badges-template').html()
    );
  ...
  onInitListener() {
    $(document).on('click', '#view-rent #emailinvoice', () => {
      ...
      bootbox.confirm(
        i18next.t('Are you sure to send invoices by email?'),
        (result) => {
          if (!result) {
            return;
          }
    ...
    $(document).on('click', '#view-rent #emailrentcall', () => {
      const tenantIds = this.getSelectedIds();
      bootbox.confirm(
        i18next.t('Are you sure to send rent notices by email?'),
        (result) => {
          if (!result) {
            return;
          }
    ...
    $(document).on('click', '#view-rent #printinvoices', () => {
      ...
      application.openPrintPreview(
        `/print/invoice/occupants/${selection}/${LOCA.currentYear}/${LOCA.currentMonth}`
      );
      return false;
    });
    ...
    $(document).on('click', '#view-rent #rentcall', () => {
      const selection = this.getSelectedIds();
      application.openPrintPreview(
        `/print/rentcall/occupants/${selection}/${LOCA.currentYear}/${LOCA.currentMonth}`
      );
      return false;
    });
    ...
    $(document).on('click', '#view-rent #recovery1', () => {
      const selection = this.getSelectedIds();
      application.openPrintPreview(
        `/print/recovery1/occupants/${selection}/${LOCA.currentYear}/${LOCA.currentMonth}`
      );
      return false;
    });
    ...
    $(document).on('click', '#view-rent #recovery2', () => {
      const selection = this.getSelectedIds();
      application.openPrintPreview(
        `/print/recovery2/occupants/${selection}/${LOCA.currentYear}/${LOCA.currentMonth}`
      );
      return false;
    });
    ...
    $(document).on('click', '#view-rent #recovery3', () => {
      const selection = this.getSelectedIds();
      application.openPrintPreview(
        `/print/recovery3/occupants/${selection}/${LOCA.currentYear}/${LOCA.currentMonth}`
      );
      return false;
    });
    ...
    $(document).on('click', '#view-rent #paymentorder', () => {
      const selection = this.getSelectedIds();
      application.openPrintPreview(
        `/print/paymentorder/occupants/${selection}/${LOCA.currentYear}/${LOCA.currentMonth}`
      );
      return false;
    });
  ...
  onUserAction($action, actionId) {
    ...
    if (actionId === 'list-action-pay-rent') {
      ...
      if (rent.occupant.terminated) {
        $('#rent-payment-form select')
          .attr('readonly', true)
          .attr('disabled', true)
          .addClass('uneditable-input');
        $('#rent-payment-form input')
          .attr('readonly', true)
          .addClass('uneditable-input');
        $('#rent-payment-form textarea')
          .attr('readonly', true)
          .addClass('uneditable-input');
  ...
  onDataChanged() {
    ...
    $monthPicker.datepicker(
      'setDate',
      moment(
        '01/' + LOCA.currentMonth + '/' + LOCA.currentYear,
        'DD/MM/YYYY'
      ).toDate()
    );
    $('#view-rent .js-rent-period').html(
      Helper.formatMonthYear(LOCA.currentMonth, LOCA.currentYear).toUpperCase()
    );

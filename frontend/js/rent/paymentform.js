...
class PaymentForm extends Form {
    ...
    beforeSetData(args) {
        ...
        if (rent.payments) {
            rent.payments = rent.payments.map(({ date, amount, type, reference }) => ({
    ...
    onGetData(data) {
        if (data.payments) {
            data.payments = data.payments
                .filter(({ paymentAmount }) => paymentAmount && Number(paymentAmount) > 0)
                .map(({ paymentDate, paymentAmount, paymentType, paymentReference }) => ({
                    date: paymentDate ? moment(paymentDate, 'L').format('DD/MM/YYYY') : '', // local format date to db format
    ...
    onBind() {
        ...
        $(domSelector + ' #btn-add-payment').off('click').click(() => {
            this.addPaymentRow();
    ...
    addPaymentRow() {
        ...
        $(domSelector + ' #payments .js-master-form-row .datepicker').datepicker('destroy');
        ...
        $('#paymentAmount_0', $newRow).attr('id', paymentAmount).attr('name', paymentAmount).val('');
        $('#paymentType_0', $newRow).attr('id', paymentType).attr('name', paymentType).val('');
        $('#paymentDate_0', $newRow).attr('id', paymentDate).attr('name', paymentDate).val('');
        $('#paymentReference_0', $newRow).attr('id', paymentReference).attr('name', paymentReference).val('');

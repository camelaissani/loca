...
export default () => {
  $.validator.addMethod(
    ...
    function (value, element, params) {
      ...
      minDate = params[0].domSelector
        ? $(params[0].domSelector).val()
        : params[0].minDate;
      ...
      params[1] = params[0].message
        ? i18next.t(params[0].message)
        : i18next.t('Please set a date after the', {
            date: momentMin.format('L'),
          });
      return (
        this.optional(element) ||
        (momentValue.isValid() &&
          momentMin.isValid() &&
          (momentValue.isSame(momentMin) || momentValue.isAfter(momentMin)))
      );
    },
    '{1}'
  );
  ...
  $.validator.addMethod(
    ...
    function (value, element, params) {
      ...
      params[1] = params[0].message
        ? i18next.t(params[0].message)
        : i18next.t('Please set a date before the', {
            date: momentMax.format('L'),
          });
      return (
        this.optional(element) ||
        (momentValue.isValid() &&
          momentMax.isValid() &&
          (momentValue.isSame(momentMax) || momentValue.isBefore(momentMax)))
      );
    },
    '{1}'
  );
  ...
  $.validator.addMethod(
    ...
    function (value, element, params) {
      ...
      if (momentBegin.isValid()) {
        if (contract === 'custom') {
          ...
          momentEnd = moment(momentBegin)
            .add(contractDuration)
            .subtract(1, 'days');
          return (
            momentValue.isValid() &&
            momentValue.isAfter(momentBegin) &&
            (momentValue.isSame(momentEnd) || momentValue.isBefore(momentEnd))
          );
        }
    ...
    i18next.t(
      'The end date of contract is not compatible with contract selected'
    )
  );
  ...
  $.validator.messages.date = i18next.t(
    'The date is not valid (Sample date:)',
    ...
    function (phone_number, element) {
      ...
      return (
        ...
        (phone_number.length > 9 &&
          phone_number.match(
            /^(?:(?:(?:00\s?|\+)33\s?)|(?:\(?0))(?:\d{2}\)?\s?\d{4}\s?\d{4}|\d{3}\)?\s?\d{3}\s?\d{3,4}|\d{4}\)?\s?(?:\d{5}|\d{3}\s?\d{3})|\d{5}\)?\s?\d{4,5})$/
          ))
      );
    },

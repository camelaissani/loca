...
class ContractDocumentsForm extends Form {
  ...
  beforeSetData(args) {
    ...
    if (occupant.documents) {
      occupant.documents.forEach((doc, index) => {
        ...
        if (index !== 0) {
          // Except first one row still exists
          this.addDocumentRow();
        }
      });
    }
  ...
  afterSetData(args) {
    ...
    $(domSelector + ' #occupantNameLabel').html(
      i18next.t("'s documents", { name: occupant.name })
    );
  }
  ...
  addDocumentRow() {
    ...
    const $newRow = $(domSelector + ' #documents .js-master-form-row')
      .clone(true)
    ...
    $('#name_0', $newRow)
      .attr('id', itemDocumentName)
      .attr('name', itemDocumentName)
      .val('');
    $('#expirationDate_0', $newRow)
      .attr('id', itemExpirtationDateName)
      .attr('name', itemExpirtationDateName)
      .val('');

  ...
  await new Promise((resolve, reject) => {
    try {
      ...
      fileref.onload = (res) => {
        if (res.type === 'error') {
          reject(
            `an error has occurred when loading the localization file ${src}`
          );
        } else {
          resolve(res);
        }
...
export default (defaultCountryCode, callback) => {
  document.addEventListener('DOMContentLoaded', () => {
    ...
    i18next.on('languageChanged', function (countryCode = defaultCountryCode) {
      ...
      try {
        Promise.all([
          updateLanguageScript(
            'moment-language',
            `//cdnjs.cloudflare.com/ajax/libs/moment.js/2.10.6/locale/${lang}.js`
          ),
          updateLanguageScript(
            'jquery-validate-language',
            `//ajax.aspnetcdn.com/ajax/jquery.validate/1.13.1/localization/messages_${langForJQueryValidate}.js`
          ),
          updateLanguageScript(
            'bootstrap-datepicker-language',
            `/node_modules/bootstrap-datepicker/dist/locales/bootstrap-datepicker.${lang}.min.js`
          ),
        ]);
      } catch (error) {
        console.error(error);

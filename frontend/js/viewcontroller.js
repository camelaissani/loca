...
class ViewController extends BaseViewMiddleware {
  ...
  initListener() {
    ...
    if (this.list) {
      this.list.on('list.selection.changed', (selection) => {
        const selectionCount =
          selection && selection.length > 0 ? selection.length : 0;
        const $selectionCardLabel = $(
          this.config.domViewId + ' .js-list-selection-menu-label'
        );

        if (selectionCount > 0) {
          const $monoSelectionActions = $(
            this.config.domViewId + ' .js-user-action.js-only-mono-selection'
          );

          if (selectionCount > 1) {
            $monoSelectionActions.addClass('disabled');
            $selectionCardLabel.html(
              i18next.t(this.config.listSelectionLabel, {
          ...
          $(this.config.domViewId + ' .js-list-selected-elements').html(
            this.templateSelectedRow({ rows: selection })
          );
    ...
    $(document).on(
      'click',
      this.config.domViewId + ' .js-user-action',
      function () {
        const $action = $(this),
        ...
        if (
          ...
          ($action.hasClass('js-only-mono-selection') &&
            self.list &&
            self.list.getSelection().length > 1)
        ) {
          return false;
        }
        ...
        if (self.list) {
          if (actionId === 'list-filter') {
            ...
            $(
              self.config.domViewId + ' .js-filterbar .js-user-action'
            ).removeClass('active');
            if (self.filterValue) {
              $(
                self.config.domViewId +
                  ' .js-filterbar .js-user-action[data-value="' +
                  self.filterValue +
                  '"]'
              ).addClass('active');
            } else {
              $(
                self.config.domViewId +
                  ' .js-filterbar .js-default-filter.js-user-action'
              ).addClass('active');
            }
  ...
  updated() {
    ...
    if (!this.loaded) {
      ...
      if (this.list) {
        this.filterValue = $(
          this.config.domViewId + ' .js-filterbar .active.js-user-action'
        ).data('value');
  ...
  loadList(callback) {
    application.httpGet(this.config.urls.overview, (req, res) => {
      ...
      $(this.config.domViewId + ' .js-all-filter-label').html(
        '(' + countAll + ')'
      );
      $(this.config.domViewId + ' .all-active-filter-label').html(
        '(' + countBusy + ')'
      );
      $(this.config.domViewId + ' .all-inactive-filter-label').html(
        '(' + countFree + ')'
      );

      $(this.config.domViewId + ' .js-filterbar .js-user-action').removeClass(
        'active'
      );
      if (this.filterValue) {
        $(
      ...
      application.httpGet(this.config.urls.items, (req, res) => {

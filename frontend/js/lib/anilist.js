...
class Anilist {
  ...
  constructor(listId, rowTemplateId, contentTemplateId) {
    ...
    // attributes
    this.listId = listId.startsWith('#')
      ? listId.slice(1, listId.length)
      : listId;
    this.rowTemplateId =
      rowTemplateId && rowTemplateId.startsWith('#')
        ? rowTemplateId.slice(1, rowTemplateId.length)
        : rowTemplateId;
    this.contentTemplateId =
      contentTemplateId && contentTemplateId.startsWith('#')
        ? contentTemplateId.slice(1, contentTemplateId.length)
        : contentTemplateId;
    this.filterText = '';
  ...
  bindDom() {
    ...
    if (rowTemplate.length > 0) {
      ...
      this.templateRowsContainer = Handlebars.compile(
        $('#' + this.contentTemplateId).html()
      );
    }
  }
  ...
  update(newDataRow) {
    ...
    $oldRow.find('.odometer').each(function () {
      ...
      self._set(
        newDataRowWithoutOdometerValues,
        key,
        self._ref(oldDataRow, key)
      );
    });
  ...
  filter(text, noAnimation, callback) {
    ...
    if (this.filterText) {
      ...
      $allRows.each(function () {
        ...
        if (
          rowFilterValues.some((value) => filterValues.indexOf(value) !== -1)
        ) {
      ...
      $rowsToShow = this.$list.find(
        '.js-list-row.list-element-filtered:hidden'
      );
  ...
  show($rows) {
    ...
    if (this.filterText) {
      $rowsToShow = $rows
        .find('.js-list-value')
        .filter(':contains("' + this.filterText + '")')
        .closest('.js-list-row:hidden');
    }
    $rowsToShow.show();
  }

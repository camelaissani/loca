...
class Helper {
  ...
  static get _Intl() {
    return {
      NumberFormat: new Intl.NumberFormat(i18next.language, {
        style: 'decimal',
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      }),
      NumberFormatPercent: new Intl.NumberFormat(i18next.language, {
        style: 'percent',
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      }),
      NumberFormatCurrency: new Intl.NumberFormat(i18next.language, {
        style: 'currency',
        currency: i18next.t('__currency_code'),
      }),
    };
  }
...
Handlebars.registerHelper('formatSurface', function (text, options) {
  ...
  text = Helper.formatSurface(
    text,
    options.hash.hideUnit,
    options.hash.emptyForZero
  );
  return new Handlebars.SafeString(text);
});
Handlebars.registerHelper('formatMoney', function (text, options) {
  ...
  if (!options) {
    ...
    html = `<span class="price-content"><span class="${classes}" data-key="${key}">${
      options.hash.hideCurrency ? amount : amountWithCurrencySymbol
    }</span></span>`;
  }
...
Handlebars.registerHelper('formatPercent', function (text, options) {
  text = Handlebars.Utils.escapeExpression(text);
  text = Helper.formatPercent(
    text,
    options.hash.hidePercent,
    options.hash.emptyForZero
  );
  return new Handlebars.SafeString(text);
});
...
Handlebars.registerHelper('paymentBadgeStatus', function () {
  ...
  if (this.status === 'paid') {
    html =
      '<span class="label label-success" data-toggle="tooltip" data-placement="bottom" title="' +
      i18next.t('Paid') +
      '"><i class="fa fa-check"></i> ' +
      moment.monthsShort()[parseInt(this.month, 10) - 1].toUpperCase() +
      '</span>';
  } else if (this.status === 'partialypaid') {
    html =
      '<span class="label label-warning" data-toggle="tooltip" data-placement="bottom" title="' +
      i18next.t('Partially paid') +
      '"><i class="fa fa-exclamation-triangle"></i> ' +
      moment.monthsShort()[parseInt(this.month, 10) - 1].toUpperCase() +
      '</span>';
  } else if (this.status === 'notpaid') {
    html =
      '<span class="label label-danger" data-toggle="tooltip" data-placement="bottom" title="' +
      i18next.t('Not paid') +
      '"><i class="fa fa-exclamation-triangle"></i> ' +
      moment.monthsShort()[parseInt(this.month, 10) - 1].toUpperCase() +
      '</span>';
...
Handlebars.registerHelper('Image', function (imageId, options) {
  ...
  if (id === 'office') {
    return new Handlebars.SafeString(
      '<i class="fa fa-home ' + cssClass + '"></i>'
    );
  }
  if (id === 'parking') {
    return new Handlebars.SafeString(
      '<i class="fa fa-car ' + cssClass + '"></i>'
    );
  }
  if (id === 'letterbox') {
    return new Handlebars.SafeString(
      '<i class="fa fa-envelope-o ' + cssClass + '"></i>'
    );
  }
  if (id === 'expiredDocument') {
    return new Handlebars.SafeString(
      '<i class="fa fa-file-text ' + cssClass + '"></i>'
    );
  }
  if (id === 'ok') {
    return new Handlebars.SafeString(
      '<i class="fa fa-thumbs-up ' + cssClass + '"></i>'
    );
  }
  if (id === 'warning') {
    return new Handlebars.SafeString(
      '<i class="fa fa-exclamation-triangle ' + cssClass + '"></i>'
    );
  }

  return new Handlebars.SafeString(
    '<i class="fa fa-question ' + cssClass + '"></i>'
  );
});

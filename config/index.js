...
const loggerLevel =
  process.env.LOCA_LOGGER_LEVEL || process.env.LOGGER_LEVEL || 'debug';
const nginxPort = process.env.NGINX_PORT || 8080;
const appHttpPort = process.env.LOCA_NODEJS_PORT || process.env.PORT || 8080;
const configDir =
  process.env.LOCA_CONFIG_DIR ||
  process.env.CONFIG_DIR ||
  path.join(__dirname, '..', 'config');
const demoMode = toBoolean(
  process.env.LOCA_DEMOMODE || process.env.DEMO_MODE || true
);
const restoreDatabase = toBoolean(process.env.RESTORE_DB || true);
const signup = toBoolean(
  process.env.LOCA_PRODUCTIVE || process.env.SIGNUP || false
);

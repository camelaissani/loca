    ...
    return new Promise((resolve, reject) => {
        ...
        occupantModel.findFilter(realm, filter, (errors, occupants) => {
            ...
            resolve(occupants
                .map(occupant => {
                    if (startTerm && endTerm) {
                        occupant.rents = occupant.rents.filter(rent => rent.term >= startTerm && rent.term <= endTerm);
                    } else if (startTerm) {
                        occupant.rents = occupant.rents.filter(rent => rent.term === startTerm);
                    }
                    return occupant;
                })
                .sort((o1, o2) => {
...
const _updateByTerm = async (realm, term, paymentData) => {
    ...
    return await new Promise((resolve, reject) => {
        const termAsNumber = Number(moment(term, 'DD/MM/YYYY HH:00').format('YYYYMMDDHH'));
        occupantModel.update(realm, dbOccupant, (errors) => {
            if (errors) {
                return reject({ errors });
            }
            const rent = dbOccupant.rents.filter(rent => rent.term === termAsNumber)[0];
...
const overview = async (req, res) => {
    try {
        ...
        const { overview } = await _getRentsDataByTerm(realm, currentDate, 'months');
        res.json(overview);
    } catch (errors) {
        logger.error(errors);
        res.status(500).json({ errors });

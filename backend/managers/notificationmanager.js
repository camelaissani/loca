        ...
        if (occupant.documents && occupant.documents.length > 0) {
          occupant.documents.forEach((document) => {
            notifications.push({
              ...
              title: occupant.name,
              description: t('has expired', {
                document: document.name,
                date: moment(document.expirationDate).format('L'),
          ...
          occupant.properties.some((p) => {
            if (
              p.property.type !== 'letterbox' &&
              p.property.type !== 'parking'
            ) {
              notifications.push({
                type: 'warning',
                notificationId: generateId(
                  occupant._id.toString() + '_no_document'
                ),
                title: occupant.name,
                description: t(
                  'There are no documents attached to the lease contract. Is the insurance certficate is missing?'
                ),
                actionUrl: '',
              });
              return true;
